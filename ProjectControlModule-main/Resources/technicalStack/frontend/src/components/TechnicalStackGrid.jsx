...
const TechnicalStackGrid = ({ projectName, version }) => {
  ...
  const handleInsertAfter = async (afterIndex) => {
    ...
    try {
      ...
      // Simple approach: add at end, then reorder if needed
      const newRow = await gridService.addRow(GRID_NAME, emptyData);
      const data = await gridService.getRows(GRID_NAME);
      setRows(data);
    ...
    <>
      ...
      <div className="rg-search-wrap">
        <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
          <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 12, zIndex: 1, color: 'var(--text-muted)', pointerEvents: 'none' }}>🔍</span>
          <input
        ...
        <span className="rg-row-count">{filteredRows.length} row{filteredRows.length !== 1 ? 's' : ''}</span>
              ...
              <tr>
                <td colSpan={INITIAL_COLUMNS.length + 1} className="rg-empty">
                  <div className="rg-empty-inner">
                    ...
                    <p>{searchTerm ? 'No results found.' : 'Grid is empty.'}</p>
                    {!searchTerm && (
                      <button className="rg-btn rg-btn-primary" onClick={handleAddRow}>
                        Add first row
                      </button>
      ...
      {contextMenu && (
        ...
        >
          ...
          <button
            ...
            onClick={() => handleDeleteRow(filteredRows[contextMenu.rowIndex]._id)}
          >
            ✕ Delete row
          </button>
        </div>
